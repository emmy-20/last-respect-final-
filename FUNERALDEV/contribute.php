...
if (!isset($_GET['death_id']) || !is_numeric($_GET['death_id'])) {
    ...
    if ($death) {
        ...
        // If no contribution, insert a pending one
        if (!$contribution) {
...
<?php if ($showList): ?>
    <div class="container-fluid">
        <h2 class="text-center mb-4">Select a Deceased Member to Contribute</h2>

        <?php if (!empty($deceasedList)): ?>
            <div class="table-responsive">
                <table class="table table-striped table-bordered text-center mx-auto" style="max-width: 800px;">
                    ...
                    <tbody>
                        <?php foreach ($deceasedList as $row): ?>
                            <tr>
                                <td><?= htmlspecialchars($row['display_name']); ?></td>
                                <td><?= htmlspecialchars($row['date_of_death']); ?></td>
                                <td>
                                    <a href="contribute.php?death_id=<?= $row['id']; ?>" class="btn btn-sm btn-primary">Contribute</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
        ...
        <?php else: ?>
            <p class="text-center text-muted">No deceased records available.</p>
        <?php endif; ?>
    </div>
<?php elseif (!$death): ?>
    <div class="container mt-5">
        <div class="alert alert-danger text-center">Death record not found.</div>
    </div>
<?php else: ?>
    <div class="container mt-5">
        ...
        <div class="card shadow p-4 mx-auto" style="color: black; max-width: 680px; margin-top: 2rem;">
            ...
            <?php if ($contribution['status'] === 'pending'): ?>
                <form action="stk_push.php" method="POST">
                    <input type="hidden" name="death_id" value="<?= $death_id; ?>" />
